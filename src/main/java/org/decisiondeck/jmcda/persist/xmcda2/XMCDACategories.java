...
public class XMCDACategories extends XMCDAHelperWithVarious {
     ...
     */
    private final Set<Category> m_categories = CollectionUtils.newLinkedHashSetNoNull();
    private final Set<Alternative> m_profiles = CollectionUtils.newHashSetNoNull();
    private final Set<Category> m_inactiveCategories = CollectionUtils.newHashSetNoNull();
	...
	for (Alternative profile : downCategories.keySet()) {
	    ...
	    catsAndProfs.setProfileUp(lower.getId(), profile);
    ...
    private Alternative read(XCategoryProfile xCategoryProfile, Map<Alternative, Category> downCategories,
	    Map<Alternative, Category> upCategories) throws InvalidInputException {
	Alternative profile;
	...
	for (Category category : categories.descendingSet()) {
	    ...
	    xCategory.setId(category.getId());
     ...
     * associations between the categories and the profiles, for each profile in the given {@link CatsAndProfs} object.
     ...
     * {@link CatsAndProfs#isComplete()}, is a sufficient condition, although not necessary.
     * </p>
     * 
     * @param catsAndProfs
     ...
     * categories with profiles, as defined in {@link CatsAndProfs#isComplete()}. In case of unexpected data, an
     * exception is thrown if this object follows the {@link ErrorManagement#THROW} strategy, otherwise, non conforming
     * informations is skipped, and an empty {@link CatsAndProfs} object is returned if this method can't figure out
	...
	/** Let's start with any category, then go down the path towards the worst category. */
	...
	/** Now let's go up again and add everything. */
	Category category = worstCategory;
	m_catsAndProfs.addCategory(category.getId());
	while (downCategories.containsValue(category)) {
	    final Alternative nextProfile = downCategories.inverse().get(category);
	    m_catsAndProfs.setProfileUp(category.getId(), nextProfile);
	    final Category nextCategory = upCategories.get(nextProfile);
	    category = nextCategory;
	    m_catsAndProfs.setCategoryUp(nextProfile, category);
	}

	if (m_catsAndProfs.getProfiles().size() != downCategories.size()) {
	    final Set<Alternative> leftOvers = Sets.difference(downCategories.keySet(), m_catsAndProfs.getProfiles());
	    assert leftOvers.size() >= 1;
	    error("The graph of neighbors from " + cStart
		    + " is not connected to the whole set of profiles and categories: " + leftOvers.iterator().next()
		    + " is unreachable.");
	...
	while (iterator.hasNext()) {
	    ...
	    assert (init != null);
	    xInitial.setCategoryID(init.getId());
	    xPair.addNewTerminal().setCategoryID(term.getId());
	    init = term;
	}
    ...
    /**
     * Retrieves a writeable view to the categories and profiles object stored in this object. The object is empty if no
     * read occurred.
     * 
     * @return not <code>null</code>.
     */
